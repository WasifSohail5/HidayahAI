...
def convert_arabic_numerals_to_western(text):
    """Convert Arabic/Urdu numerals to Western digits"""
    numeral_map = {
        '٠': '0', '۰': '0',
        '١': '1', '۱': '1',
        '٢': '2', '۲': '2',
    ...
    for arabic, western in numeral_map.items():
...
def extract_quran_references(text):
    """Extract Quran references from text (including Urdu)."""
    # First convert any Arabic/Urdu numerals to Western digits
    text = convert_arabic_numerals_to_western(text)
    references = []
    ...
    # Urdu patterns
    # Pattern for "سورۃ البقرۃ، آیت ٢٥٥" or "سورہ البقرہ، آیت ٢٥٥"
    urdu_pattern1 = re.compile(r"سورۃ\s+([^،]+)،\s+آیت\s+(\d+)")
    matches = urdu_pattern1.findall(text)
    references.extend([("quran", surah, ayah) for surah, ayah in matches])

    urdu_pattern2 = re.compile(r"سورہ\s+([^،]+)،\s+آیت\s+(\d+)")
    matches = urdu_pattern2.findall(text)
    references.extend([("quran", surah, ayah) for surah, ayah in matches])

    # Pattern without comma
    urdu_pattern3 = re.compile(r"سورۃ\s+([^\d]+)\s+آیت\s+(\d+)")
    matches = urdu_pattern3.findall(text)
    references.extend([("quran", surah.strip(), ayah) for surah, ayah in matches])

    urdu_pattern4 = re.compile(r"سورہ\s+([^\d]+)\s+آیت\s+(\d+)")
    matches = urdu_pattern4.findall(text)
    references.extend([("quran", surah.strip(), ayah) for surah, ayah in matches])

    # Direct citation pattern (قرآن ٢:٢٥٥)
    urdu_pattern5 = re.compile(r"قرآن\s+(\d+):(\d+)")
    matches = urdu_pattern5.findall(text)
    references.extend([("quran", surah, ayah) for surah, ayah in matches])
...
def extract_hadith_references(text):
    """Extract hadith references from text (including Urdu)."""
    # First convert any Arabic/Urdu numerals to Western digits
    text = convert_arabic_numerals_to_western(text)
    references = []
    ...
    # Urdu patterns
    # Pattern for "صحیح بخاری، حدیث ١٢٣"
    urdu_pattern1 = re.compile(
        r"(صحیح بخاری|صحیح مسلم|سنن ابو داؤد|جامع الترمذی|سنن النسائی|سنن ابن ماجہ)[،\s]+حدیث\s+(\d+)")
    matches = urdu_pattern1.findall(text)
    references.extend([("hadith", convert_urdu_book_name(book), number) for book, number in matches])

    # Pattern for "بخاری ١٢٣"
    urdu_pattern2 = re.compile(r"(بخاری|مسلم|ابو داؤد|ترمذی|نسائی|ابن ماجہ)\s+(\d+)")
    matches = urdu_pattern2.findall(text)
    references.extend([("hadith", convert_urdu_book_name(book), number) for book, number in matches])
...
def convert_urdu_book_name(urdu_name):
    """Convert Urdu hadith book names to English equivalents"""
    urdu_to_english = {
        'صحیح بخاری': 'Sahih Bukhari',
        'بخاری': 'Bukhari',
        'صحیح مسلم': 'Sahih Muslim',
        'مسلم': 'Muslim',
        'سنن ابو داؤد': 'Sunan Abu Dawood',
        'ابو داؤد': 'Abu Dawood',
        'جامع الترمذی': 'Jami at-Tirmidhi',
        'ترمذی': 'Tirmidhi',
        'سنن النسائی': 'Sunan an-Nasai',
        'نسائی': 'Nasai',
        'سنن ابن ماجہ': 'Sunan Ibn Majah',
        'ابن ماجہ': 'Ibn Majah'
    }

    return urdu_to_english.get(urdu_name, urdu_name)
...
def ensure_reference_format_consistency(text):
    ...
    # Urdu patterns
    # For Quran in Urdu: Make sure it's "سورہ X، آیت Y"
    urdu_surah_pattern = re.compile(r"(سورۃ\s+[^\d،]+)\s+آیت")
    text = urdu_surah_pattern.sub(r"\1، آیت", text)

    urdu_surah_pattern2 = re.compile(r"(سورہ\s+[^\d،]+)\s+آیت")
    text = urdu_surah_pattern2.sub(r"\1، آیت", text)

    # For Hadith in Urdu: Make sure it's "صحیح بخاری، حدیث Y"
    urdu_hadith_pattern = re.compile(
        r"(صحیح بخاری|صحیح مسلم|سنن ابو داؤد|جامع الترمذی|سنن النسائی|سنن ابن ماجہ)\s+حدیث")
    text = urdu_hadith_pattern.sub(r"\1، حدیث", text)
...
def add_links_to_references(text):
    """Add links to all references in text (including Urdu)."""
    result = text
        ...
        if ref_type == "quran":
            ...
            if link:
                ...
                ref_forms = [
                    ...
                    f"Quran {surah}:{ayah}",
                    # Urdu forms
                    f"سورۃ {surah}، آیت {ayah}",
                    f"سورۃ {surah} آیت {ayah}",
                    f"سورہ {surah}، آیت {ayah}",
                    f"سورہ {surah} آیت {ayah}",
                    f"قرآن {surah}:{ayah}",
                ]
            ...
            # Get Urdu equivalent of book name if it exists
            urdu_book = get_urdu_book_name(book)
    ...
    # For Urdu Quran references
    urdu_quran_pattern = re.compile(r"سورۃ\s+([^،]+)،\s+آیت\s+(\d+)(?!\s*\(http)")
    for match in urdu_quran_pattern.finditer(result):
        surah, ayah = match.groups()
        ref_text = match.group(0)
        link = build_quran_link(surah, ayah)
        if link:
            result = result.replace(ref_text, f"{ref_text} ({link})")
            logger.info(f"Added missing Urdu Quran link: {ref_text} -> {link}")

    urdu_quran_pattern2 = re.compile(r"سورہ\s+([^،]+)،\s+آیت\s+(\d+)(?!\s*\(http)")
    for match in urdu_quran_pattern2.finditer(result):
        surah, ayah = match.groups()
        ref_text = match.group(0)
        link = build_quran_link(surah, ayah)
        if link:
    ...
    # For Urdu Hadith references
    urdu_hadith_pattern = re.compile(
        r"(صحیح بخاری|صحیح مسلم|سنن ابو داؤد|جامع الترمذی|سنن النسائی|سنن ابن ماجہ)[،\s]+حدیث\s+(\d+)(?!\s*\(http)")
    for match in urdu_hadith_pattern.finditer(result):
        book, number = match.groups()
        ref_text = match.group(0)
        link = build_hadith_link(convert_urdu_book_name(book), number)
        result = result.replace(ref_text, f"{ref_text} ({link})")
        logger.info(f"Added missing Urdu Hadith link: {ref_text} -> {link}")

    return result


def get_urdu_book_name(english_name):
    """Get Urdu name for English hadith book name"""
    english_to_urdu = {
    ...
    return english_to_urdu.get(english_name, english_name)
...
async def process_islamic_query(query: str, source_type: str = "auto", top_k: int = 10):
    ...
    prompt = f"""
        ...
        4. Always include direct references for every claim:
           ...
           - **In Urdu**: 
              - Quran: Format exactly like "سورہ [Name]، آیت [Number]"
              - Hadith: Format exactly like "[Full Book Name Urdu]، حدیث [Number]"
        5. Use only these exact book names:
           - In English: "Sahih Bukhari", "Sahih Muslim", "Sunan Abu Dawood", "Jami at-Tirmidhi", "Sunan an-Nasai", "Sunan Ibn Majah"
           - In Urdu: "صحیح بخاری", "صحیح مسلم", "سنن ابو داؤد", "جامع الترمذی", "سنن النسائی", "سنن ابن ماجہ"
        ...
        7. If nothing directly matches, check for related concepts and **never reject a question too early**. Clearly say: 
           - In English: "No exact mention found, but related Hadith/Quranic references include…"
           - In Urdu: "اس کا کوئی براہ راست ذکر نہیں ملا، لیکن متعلقہ حدیث/قرآنی حوالے شامل ہیں..."
